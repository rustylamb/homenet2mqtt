...
`decode` 및 `value_encode`에서 사용할 수 있는 특수한 인코딩 타입에 대한 상세 설명입니다.

### signed_byte_half_degree

일부 월패드(난방기)에서 사용하는 독특한 온도 인코딩 방식입니다. 1바이트를 사용하여 0.5도 단위의 정밀도와 부호를 표현합니다.

**비트 구조:**
*   `Bit 7` (0x80): 0.5도 플래그 (1이면 +0.5도)
*   `Bit 6` (0x40): 부호 비트 (1이면 음수, 단 `signed: true`일 때만 적용)
*   `Bit 0-6` (0x7F): 기본 정수 값 (비트 6 포함)

**계산 로직:**
1.  **기본 값**: `Value = Byte & 0x7F` (0~127)
2.  **0.5도 추가**: 만약 `Byte & 0x80`이 1이면, `Value += 0.5`
3.  **부호 반전**: 만약 `signed: true`이고 `Byte & 0x40`이 1이면, `Value = -Value`

**예시 (`signed: true`일 때):**
*   `0x19` (0001 1001) → 25도
*   `0x99` (1001 1001) → 25.5도 (`0x19` + 0.5)
*   `0x45` (0100 0101) → -69.0도 (`0x45`=69, 부호 반전)
*   `0xC5` (1100 0101) → -69.5도 (`0x45`=69 + 0.5, 부호 반전)

> **주의**: 이 방식에서 `Bit 6`은 값의 일부이면서 동시에 부호 플래그로 사용됩니다. 따라서 음수 표현 방식이 표준 2의 보수법과 다릅니다.
