...
export const globalSecurityHeaders = (_req: Request, res: Response, next: NextFunction) => {
  ...
  res.setHeader(
    ...
    "default-src 'self'; script-src 'self' 'unsafe-inline' https://giscus.app; style-src 'self' 'unsafe-inline' https://giscus.app; img-src 'self' data: https://*.githubusercontent.com; connect-src 'self' ws: wss: https://raw.githubusercontent.com https://giscus.app; frame-src 'self' https://giscus.app; object-src 'none'; base-uri 'self'; form-action 'self';",
  );
  // 민감한 브라우저 기능 비활성화
